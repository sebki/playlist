...
func (m GameModel) Insert(game *Game) error {
	query := `
		INSERT INTO games (title, descr, year_published, bg_type, thumbnail, image, min_player, max_player, min_playtime, max_playtime, min_age, max_age)
...
func (m GameModel) Get(id int64) (*Game, error) {
	...
	query := `
		SELECT id, created_at, modified_at, title, descr, year_published, bg_type, thumbnail, image, min_player, max_player, min_playtime, max_playtime, min_age, max_age, version
		FROM games
		WHERE id = $1
	`
...
func (m GameModel) Update(game *Game) error {
	...
	query := `
		UPDATE game 
		SET modified_at = transaction_timestamp(), title = $1, desc = $2, year_published= $3, bg_type = $4, thumbnail = $5, image = $6, min_player = $7, max_player = $8, min_playtime = $9, max_playtime = $10, min_age = $11, max_age = $12, version = version + 1
...
func (m GameModel) GetAll(title string, bgType []string, filters Filters) ([]*Game, Metadata, error) {
	query := fmt.Sprintf(`
		SELECT count(*) OVER(), id, created_at, modified_at, title, descr, year_published, bg_type, thumbnail, image, min_player, max_player, min_playtime, max_playtime, min_age, max_age, version
		FROM games
